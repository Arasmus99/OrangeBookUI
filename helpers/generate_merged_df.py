...
def scrape_fda_novel_approvals(year):
    if 2021 <= year <= 2025:
        ...
        raise ValueError("Year must be between 2015 and 2025.")
...
def parse_patent_google(patent_number):
    ...
    keywords = {"crystalline": ["crystalline"], "amorphous": ["amorphous", "solid dispersion", "disordered"], "salt": ["salt"]}
    return {"Crystalline": found(claims, keywords["crystalline"]), "SF": found(claims, keywords["crystalline"]), "Amorphous": found(claims, keywords["amorphous"]), "Salt": found(claims, keywords["salt"]), "Assignee": assignee, "Claims": claims}
...
def generate_merged_df(year):
    ...
    columns_to_drop = ["index", "DF;Route", "Trade_Name", "Applicant", "Strength", "Appl_Type_x", "Appl_No", "Product_No", "TE_Code", "RLD", "RS", "Type", "Appl_Type_y", "Patent_Use_Code", "Delist_Flag", "Submission_Date"]
    merged_df.drop(columns=[col for col in columns_to_drop if col in merged_df.columns], inplace=True)
    merged_df.rename(columns={"Patent_Expire_Date_Text": "Expiration Date", "Drug_Substance_Flag": "DS", "Drug_Product_Flag": "DP", "Applicant_Full_Name": "NDA Holder", "Patent_No": "Patent Number", "Ingredient": "Active Ingredient", "normalized_trade_name": "Drug Name"}, inplace=True)
    desired_order = ["Drug Name", "Active Ingredient", "Approval_Date", "Patent Number", "Expiration Date", "DS", "DP", "SF", "Crystalline", "Amorphous", "Salt", "NDA Holder", "Assignee", "Claims"]
